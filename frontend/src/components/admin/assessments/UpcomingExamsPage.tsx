...
const UpcomingExamsPage: React.FC = () => {
  ...
  const upcomingExams = useMemo(() => {
    return allExams.filter(exam => exam.status === 'scheduled');
  }, [allExams]);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Upcoming Exams</h1>
          <p className="text-gray-600">View and manage scheduled exams</p>
        </div>
        <button
          onClick={() => navigate(-1)}
